...
export default function fillEmptySpace(tune) {
  ...
  newTune.parts = tune.parts.map((part) => {
    ...
    for (let line = getLine(part.bars, cursor); line.length > 0; line = getLine(part.bars, cursor)) {
      ...
      /*
        this could also be: if the line in its unexpanded form takes up less than half the space of
        the renderWidth, don't expand it.
      */
      ...
      if (line[0] !== part.bars[part.bars.length - 1]) {
        ...
        cursor += line.length;
        outputBars = outputBars.concat(expandedLine); // how is this for immutability?
      } else {
