...
export default function setBarPositions(tune) {
  ...
  newTune.parts = tune.parts.map((part, partIndex) => {
    /*
      ...
      actually can't freeze things because I can't copy them because when they contain VexFlow
      objects they'll contain circular references?
    */
    const newPart = Object.assign({}, part);
    newPart.bars = newPart.bars.map((bar, i) => {
      // const newBar = merge({}, bar); // cant do this bcause of circular references
      ...
      // this should be handled in a way that makes sense
      newBar.notes.forEach((note) => {
        if (note.dots === 1) {
          minWidth += dottedNotesModifier;
        }
        if (note.duration === '2') { // won't help dotted half??? or will it anyway...
          minWidth += dottedNotesModifier;
        }
      });
