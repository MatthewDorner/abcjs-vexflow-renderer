...
export default function setBarPositions(tune) {
  const {
    renderWidth, xOffset, widthFactor, lineHeight, clefWidth, meterWidth, repeatWidthModifier, keySigAccidentalWidth
  } = tune.renderOptions;
      ...
      if (i === 0) { // first bar of part
        ...
        if (tune.renderOptions.tabsVisibility || partIndex === 0) {
          if (partIndex === 0) {
            ...
            yCursor += (lineHeight * ((14 - (6 - tune.renderOptions.tuning.tuning.length)) / 14)) * 1.25;
          }
        } else {
        ...
        if (tune.renderOptions.tabsVisibility) {
          yCursor += (lineHeight * ((14 - (6 - tune.renderOptions.tuning.tuning.length)) / 14));
        } else {
