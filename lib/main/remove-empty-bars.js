  ...
  newTune.parts = newTune.parts.map((part, partIndex) => {
    ...
    newPart.bars = part.bars.filter((bar, i) => {
      ...
      if (newBar.notes.length === 0) {
        if (newBar.vexKeySignature !== '') {
          if (i + 1 < newPart.bars.length) { // fiddle hill jig fails
            // THIS IS GONNA FAIL NOW BECAUSE OF USING THESE 'new' OBJECTS
            // BECUASE I CAN'T MUTATE THE i + 1... right. or can I? yeah...
            newPart.bars[i + 1].vexKeySignature = newBar.vexKeySignature;
            ...
            newTune.parts[partIndex + 1].bars[0].vexKeySignature = newBar.vexKeySignature;
