  ...
  partRegions.forEach((partRegion, partIndex) => {
    ...
    partRegion.barRegions.forEach((barRegion, barIndex) => {
      ...
      barRegion.contents.forEach((obj, objIndex) => {
        ...
        const { duration, isDotted } = getVexDuration(obj.duration);
        let noteToAdd;

        // CREATE AND ADD MODIFIERS TO STAVE NOTE / REST
        if (!obj.rest) {
          const keys = getKeys(obj.pitches);
          const accidentals = obj.pitches.map(pitch => VEX_ACCIDENTAL_FROM_ABCJS[pitch.accidental]);

          noteToAdd = new StaveNote({
            clef: tuneAttrs.clef, keys, duration, auto_stem: true
          });
          addDecorations(noteToAdd, obj.decoration);
          ...
          if (obj.gracenotes) {
            ...
            obj.gracenotes.forEach((graceNote) => {
              const graceKey = getKeys([graceNote])[0];
              const graceAccidental = VEX_ACCIDENTAL_FROM_ABCJS[graceNote.accidental];
              const vexDuration = getVexDuration(graceNote.duration);
          ...
          const tabNoteToAdd = new TabNote({
            positions: getTabPosition(keys, currentAbcKeySignature, barRegion.contents, objIndex, renderOptions.tuning, false),
            duration
          });
          ...
          if (obj.gracenotes) {
            ...
            obj.gracenotes.forEach((graceNote) => {
              ...
              const graceNoteToAdd = new GraceTabNote({
                positions: getTabPosition([graceKey], currentAbcKeySignature, barRegion.contents, objIndex, true),
          ...
          // TODO: draw an invisible rest somehow? Since drawing the rest causes problem with whistle and
          // harmonica tab where the tab stave isn't big enough to draw the rest and it ends up extending
          // below the bottom of the tab stave
        ...
        if (obj.chord) {
          ...
          } // sharps don't cause a problem, the '♭' symbol wont' be displayed though
