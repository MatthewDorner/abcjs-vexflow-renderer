 ...
 */
export default function getTabPosition(keys, abcKeySignature, barContents, i, isGraceNote) {
  ...
  barContents.forEach((previousObj, j) => {
    if (previousObj.el_type === 'note' && !previousObj.rest) {
      ...
      //  to stop a note's accidental from applying to its own attached grace notes
      const previousAppliesToCurrent = (i >= j && !isGraceNote) || (i > j);
      if (previousAppliesToCurrent && getAccidentals(previousObj.pitches)[0]) {
  ...
  // apply accidentals from previous grace notes or this grace note. could've been done in the same
  // loop above but this is easier to think about
  barContents.forEach((previousObj, j) => {
    if (previousObj.gracenotes) {
      const previousKeys = getKeys(previousObj.gracenotes);
      if (i >= j && getAccidentals(previousObj.gracenotes)[0]) {
        if (diatonicNote === NoteUtils.getDiatonicFromLetter(previousKeys[0])) {
          adjustedChromaticNote = chromaticNote + NoteUtils.getSemitonesForAccidental(getAccidentals(previousObj.gracenotes)[0]);
        }
      }
    }
  });
