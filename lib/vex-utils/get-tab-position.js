 ...
 * @param   {Array} tuning the tuning from lowest string to highest in VexFlow format
 * such as ['e/3', 'a/3', 'd/4', 'g/4', 'b/4', 'e/5']
...
export default function getTabPosition(keys, abcKeySignature, barContents, index, tuning, isGraceNote) {
  ...
  const stringNumbers = tuning.map(string => getAbsoluteChromatic(string));
  let left;
  let top;
