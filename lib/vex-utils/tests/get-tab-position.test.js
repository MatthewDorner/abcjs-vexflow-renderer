...
test('e3 is lowest note on guitar', () => {
  ...
  expect(VexUtils.getTabPosition(keys, abcKeySignature, barContents, 0, false)).toStrictEqual([{ str: 6, fret: 0 }]);
});
...
test('sharped note causes itself and later notes to be sharped', () => {
  ...
  expect(VexUtils.getTabPosition(keys, abcKeySignature, barContents, 0, false)).toStrictEqual([{ str: 1, fret: 1 }]);
  expect(VexUtils.getTabPosition(keys, abcKeySignature, barContents, 1, false)).toStrictEqual([{ str: 1, fret: 2 }]);
  expect(VexUtils.getTabPosition(keys, abcKeySignature, barContents, 2, false)).toStrictEqual([{ str: 1, fret: 2 }]);
});
...
test('sharp in key signature causes note to be sharped', () => {
  ...
  expect(VexUtils.getTabPosition(keys, abcKeySignature, barContents, 0, false)).toStrictEqual([{ str: 1, fret: 2 }]);
});
...
test('in B Flat Minor key, B notes get flatted', () => {
  ...
  expect(VexUtils.getTabPosition(keys, abcKeySignature, barContents, 0, false)).toStrictEqual([{ str: 1, fret: 6 }]);
  // now with C major key sig, should be a B natural
  expect(VexUtils.getTabPosition(keys, AbcKeySignatures.CMajor, barContents, 0, false)).toStrictEqual([{ str: 1, fret: 7 }]);
});
...
test('grace note - sharped grace note causes itself and later grace notes to be sharped', () => {
  ...
  expect(VexUtils.getTabPosition(keys, abcKeySignature, barContents, 0, true)).toStrictEqual([{ str: 1, fret: 1 }]);
  expect(VexUtils.getTabPosition(keys, abcKeySignature, barContents, 1, true)).toStrictEqual([{ str: 1, fret: 2 }]);
  expect(VexUtils.getTabPosition(keys, abcKeySignature, barContents, 2, true)).toStrictEqual([{ str: 1, fret: 2 }]);
});
...
test('grace note - sharp in key signature causes grace note to be sharped', () => {
  ...
  // str: 1, fret: 2 is F sharp
  expect(VexUtils.getTabPosition(keys, abcKeySignature, barContents, 0, true)).toStrictEqual([{ str: 1, fret: 2 }]);
});
